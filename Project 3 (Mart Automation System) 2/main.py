...
def dashinfo():
    if "user" in session and session['user']==params['nor_user']:
            posts = Purchase_info.query.all()
            return render_template("dash_info.html",params=params,purchase=posts)
    else:
            return redirect("error.html")
...
def editu(sno):
    if "user" in session and session['user']==params['nor_user']:
        if request.method == 'POST':
                ...
                if sno == '0':
                    post = Purchase_info(product_id = box_product_id,product_name = box_product_name, product_mrp = box_product_mrp ,product_quant = box_product_quantity)
                    db.session.add(post)
                    db.session.commit()
                    return redirect('/dashinfo')
                else:
...
def ed(product_id):
            ...
            if  post != "":
                    ...
                    pur =  Purchase_info(product_id = pr_id, product_name = pr_name, product_mrp = pr_mrp,product_quant = pr_quant )
                    db.session.add(pur)
                    db.session.commit()
...
def bar(sno):
    if "user" in session and session['user']==params['user_name']:
        ...
        Hr =hr("http://192.168.0.101:5000/0/"+pro_id,writer=ImageWriter())
        Hr.save("C:/Users/Vishant-PC/Desktop/Programs/FLASK/Project 3 (Mart Automation System)/static/Assets/imgs/Id")
